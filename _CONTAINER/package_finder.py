...
class PackageFinder:
    ...
    def get_install_candidate(
        ...
        return InstallationCandidate(
            name=link_evaluator.project_name, link=link, version=result
        )
    ...
    def find_all_candidates(self, project_name: str) -> List[InstallationCandidate]:
        ...
        collected_sources = self._link_collector.collect_sources(
            ...
            candidates_from_page=functools.partial(
                self.process_project_url, link_evaluator=link_evaluator
            ),
        )
        ...
        file_candidates = self.evaluate_links(
            link_evaluator, sorted(file_links_it, reverse=True)
        )
    ...
    def find_best_candidate(
        ...
        candidate_evaluator = self.make_candidate_evaluator(
            project_name=project_name, specifier=specifier, hashes=hashes
        )
    ...
    def find_requirement(
        ...
        def _format_versions(cand_iter: Iterable[InstallationCandidate]) -> str:
            ...
            return (
                ", ".join(
                    sorted({str(c.version) for c in cand_iter}, key=parse_version)
                )
                or "none"
            )
