...
class game_ball:
    ...
    def move(self):
        ...
        if self.rect.colliderect(player_paddle):
            # Collision from top
            if (
                abs(self.rect.bottom - player_paddle.rect.top) < collision_thresh
                and self.speed_y > 0
            ):
                self.speed_y *= -1
...
# Instance of ball
ball = game_ball(
    player_paddle.x + (player_paddle.width // 2), player_paddle.y - player_paddle.height
)
...
while run:
    ...
    if not live_ball:
        if game_over == 0:
            draw_text(
                "CLICK ANYWHERE TO START", font, text_col, 300, screen_height // 2 + 100
            )
        elif game_over == 1:
