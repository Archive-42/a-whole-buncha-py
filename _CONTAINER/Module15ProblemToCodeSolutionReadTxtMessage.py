...
def mainFunction():
    ...
    if fileFound == 0:
        ...
        translatedMessage = compare(
            messageWords, allAbbreviations, allTranslations
        )  # the final list of words for the message

        # Step 4) Print the translated message to the user
        print(translatedMessage)
    else:
        print(
            "Could not translate message, file containing translation terms could not be located."
        )
