...
print("")
SignUp_Choice = input(
    "Do you want to create a new account type YES to create a new account or type no to login :"
)
print("")

if (
    SignUp_Choice == "no"
):  # If choice is no the options to book,etc will be available after entering the username and password
    Enter_username = input("Enter UserName:")
    Enter_Password = getpass.getpass(
        "Enter Password:"
    )  # Getpass library used to hide the password typed by the user
    cursor.execute(
        """SELECT Create_Password FROM customer_info 
                             WHERE Username= %s""",
        (Enter_username,),
    )  # Checking if the login information entered exists in the database
    result = cursor.fetchone()
    if (
        result == None
    ):  # If the entered password or username is wrong or the username doesnot exists the error message is displayed
    ...
    for row1 in result:
        ...
        if (
            Data == Enter_Password
        ):  # If the information is enetred is correct the options are shown
            ...
            if option_selection == 1:
                ...
                print("")
                a1 = [
                    "Alu+Roti(6 pic)+Dahi+Chatni+DalPalak+Rice+Salad",
                    "DamAlu+MethiParatha+DalRice+Salad",
                    "MutterPanner+Roti(6 piece)Dal+Pulao+Salad",
                    "Chole+Puri+Dal+Rice+Salad+GreenVagitables+FruitSalad",
                    "Paneer+Roti+Sambar+Roti+Salad",
                    "MixVeg Dal+PlaneRice+Salad+Roti(6 piece)+Shira",
                    "Rajma+Dal+Roti(6 piece)+Rice+Onion",
                ]
                table1 = {
                    "DAYS": [
                        "Monday",
                    ...
                    "BREAKFAST (7:00 AM -10:00 AM)": [
                        "Puri+Aloo chana Sabji",
                        "Plane Paratha + Aloo Sabji",
                        " Poha",
                        "Upma",
                        "Puri + Aloo Sbji",
                        "Paratha + Chatni ",
                    ...
                    "LUNCH (12:00-2:00 PM)": [
                        "Rice+Dal+Sabji+Salad+Roti ",
                        "Rice+AlooGovi Sabji +Dal+Salad",
                        "Rice+Egg+Curry+Dal+Roti(7+Salad)",
                        "Rice + Rajma+Dal",
                        "Rice+Green Vagitables+Roti+Chips",
                        "FriedRice+Dal+Chatni+Roti",
                        "Rice+Dal+Roti+Salad",
                    ],
                ...
                print("")
                print(
                    "Press the correspinding number and press enter each time to add items"
                )
                print(
                    "If you are done adding items then enter 8 as the item choice and press enter,your above choices will be recorded"
                )
                print(
                    "WARNING! Your Previously entered choices for tiffin will be deleted please enter all your choices again as per above MENU "
                )
                print("")
                ...
                Tiffin_Choice = 0
                cursor.execute(
                    """DELETE FROM User_TiffinChoice WHERE Username= %s""",
                    (Enter_username,),
                )
                ...
                cursor.execute(
                    """SELECT SUM(b.Cost) FROM User_TiffinChoice a JOIN Tiffin_Type b ON a.Tiffin_Choice_Id=b.Tiffin_Id WHERE a.Username= %s""",
                    (Enter_username,),
                )
                ...
                print(
                    "Enter the corresponding day number to select days on which tiffin is to be delivered"
                )
                print("")
                print(
                    """
                ...
                Day_Choice = 0
                cursor.execute(
                    """DELETE FROM User_TiffinDays WHERE Username= %s""",
                    (Enter_username,),
                )
                while Day_Choice != 8:
                ...
                while Order_Summary != "y":
                    ...
                    if Order_Summary == "y":
                        ...
                        for name in result_name:
                            print("Name:", name)
                        cursor.execute(
                            """SELECT Address FROM customer_info
                              WHERE Username= %s""",
                            (Enter_username,),
                        )
                        ...
                        cursor.execute(
                            """SELECT Contact FROM customer_info
                              WHERE Username= %s""",
                            (Enter_username,),
                        )
                        ...
                        cursor.execute(
                            """SELECT Tiffin_Type.Tiffin_Id,Tiffin_Type.Tiffin,Tiffin_Type.Cost
                              FROM Tiffin_Type
                              INNER JOIN User_TiffinChoice ON
                              Tiffin_Type.Tiffin_Id=User_TiffinChoice.Tiffin_Choice_Id
                              WHERE User_TiffinChoice.Username  = %s""",
                            (Enter_username,),
                        )
                        ...
                        cursor.execute(
                            "SELECT Tfiini_Days_Choice FROM User_TiffinDays WHERE Username=%s",
                            (Enter_username,),
                        )
                ...
                print("")
                OrderConfirmation_Choice = input(
                    "Type yes if you want to confirm your orders or press no to exit:"
                )
                if OrderConfirmation_Choice == "yes":
                    cursor.execute(
                        """SELECT User_Id FROM customer_info WHERE Username = %s""",
                        (Enter_username,),
                    )
                    ...
                    sql_syntax_sum = (
                        "INSERT INTO User_TotalCost (User_cost,Username) VALUES(%s,%s)"
                    )
                    ...
                    cursor.execute(
                        """SELECT Order_ID FROM Delivery_info WHERE Customer_ID = %s""",
                        (ID,),
                    )
            ...
            if option_selection == 2:
                ...
                cursor.execute(
                    """SELECT Order_ID FROM Delivery_info WHERE Order_ID = %s  """,
                    (order_id_input,),
                )
                ...
                if order_id_input == orderid_data:
                    cursor.execute(
                        """SELECT Order_Status FROM Delivery_info
                              WHERE Order_ID= %s""",
                        (order_id_input,),
                    )
                    ...
                    cursor.execute(
                        """SELECT DeliverOrder_Date FROM Delivery_info
                              WHERE Order_ID= %s""",
                        (order_id_input,),
                    )
            ...
            if option_selection == 3:
                ...
                if result_order_id_output1 == None:
                    print(
                        "OrderID entered is incorrect or you have not booked any tiffin Please restart the program"
                    )
                    exit()
                ...
                if order_id_input1 == orderid_data1:
                    ...
                    cursor.execute(
                        """SELECT User_cost FROM User_TotalCost WHERE Username=%s""",
                        (Enter_username,),
                    )
                    ...
                    print(" ")
                    cursor.execute(
                        """SELECT TiffinBoy_Name FROM Delivery_info
                                 WHERE Order_ID= %s""",
                        (order_id_input1,),
                    )
                    ...
                    cursor.execute(
                        """SELECT TiffinBoy_Phone_Number FROM Delivery_info
                                 WHERE Order_ID= %s""",
                        (order_id_input1,),
                    )
            ...
            if option_selection == 6:
                ...
                cursor.execute(
                    """SELECT Master_Password FROM Master_Login
                         WHERE Master_Username= %s""",
                    (Enter_master_username,),
                )
                ...
                cursor.execute(
                    """SELECT Master_Username FROM Master_Login
                        WHERE Master_Password= %s""",
                ...
                for row1_master in result_master:
                    ...
                    if Data_master == Enter_master_Password:
                        ...
                        cursor.execute(
                            "SELECT SUM(b.Cost) FROM User_TiffinChoice a JOIN Tiffin_Type b ON a.Tiffin_Choice_Id=b.Tiffin_Id"
                        )
            ...
            if option_selection == 5:
                delete_tiffinsubscription = input(
                    "Do You reall want to unsubscribe to the tiffin service with Username "
                    + str(Enter_username)
                    + " (yes/no):"
                )
                if delete_tiffinsubscription == "yes":
                    ...
                    for finalresult_userid in result_for_userid:
                        ...
                        mydb.commit()
                        delete_statement1 = (
                            """DELETE FROM Delivery_info WHERE Customer_ID=%s"""
                        )
            ...
            if option_selection == 4:
                print(
                    " What Do You Want To Update ? Press the corresponding number to navigate WARNING! You can update only one detail at a time please restart the programm to update multiple details"
                )
                print(
                    """
                        1 == Name
                ...
                if update_user_choice == 1:
                    ...
                    newname_statement = (
                        """UPDATE customer_info SET Name=%s WHERE Username =%s"""
                    )
                ...
                if update_user_choice == 2:
                    ...
                    newaddress_statement = (
                        """UPDATE customer_info SET Address=%s WHERE Username =%s"""
                    )
                ...
                if update_user_choice == 3:
                    ...
                    newcontact_statement = (
                        """UPDATE customer_info SET Contact=%s WHERE Username =%s"""
                    )
                ...
                if update_user_choice == 4:
                    ...
                    newmail_statement = (
                        """UPDATE customer_info SET Email=%s WHERE Username =%s"""
                    )
...
# From here the else statement of the very first if statement starts i.e, if the user chooses to signup
    ...
    print(
        "Please enter the following information and PRESS ENTER to proceed"
    )  # User is asked to enter the details which is stored in the database
    print("")
...
)  # When all the data is succefully stored in the database the user is required to login at this point
print("")
...
if Enter_username == Username and Enter_Password == Create_Password:
    ...
    if option_selection == 1:
        ...
        Tiffin_Choice = 0
        cursor.execute(
            """DELETE FROM User_TiffinChoice WHERE Username= %s""", (Enter_username,)
        )
        while Tiffin_Choice != 8:
            Tiffin_Choice = int(input("Enter Item Number:"))
            if Tiffin_Choice == 1:
        ...
        Day_Choice = 0
        cursor.execute(
            """DELETE FROM User_TiffinDays WHERE Username= %s""", (Enter_username,)
        )
        while Day_Choice != 8:
            Day_Choice = int(input("Enter Day Number:"))
            if Day_Choice == 1:
        ...
        if OrderConfirmation_Choice == "yes":
            ...
            cursor.execute(
                """SELECT Order_ID FROM Delivery_info WHERE Customer_ID = %s""", (ID,)
            )
            result_orderid = cursor.fetchone()
            for print_orderid in result_orderid:
                show_orderid = print_orderid
    ...
    if option_selection == 4:
        ...
        if update_user_choice == 1:
            ...
            for finalresult_userid1 in result_for_userid1:
                finalresult1_userid1 = finalresult_userid1
            newname_statement1 = (
                """UPDATE Delivery_info SET Customer_Name=%s WHERE Customer_ID =%s"""
            )
            values_name1 = (new_name, finalresult1_userid1)
            cursor.execute(newname_statement1, values_name1)
            mydb.commit()
        ...
        if update_user_choice == 2:
            ...
            for finalresult_userid2 in result_for_userid2:
                finalresult1_userid2 = finalresult_userid2
            newaddress_statement1 = (
                """UPDATE Delivery_info SET Delivery_Address=%s WHERE Customer_ID =%s"""
            )
            values_address1 = (new_address, finalresult_userid2)
            cursor.execute(newaddress_statement1, values_address1)
            mydb.commit()
        ...
        if update_user_choice == 3:
            ...
            for finalresult_userid3 in result_for_userid3:
                finalresult1_userid3 = finalresult_userid3
            newcontact_statement1 = (
                """UPDATE Delivery_info SET Customer_Contact=%s WHERE Customer_ID =%s"""
            )
            values_contact1 = (new_contact, finalresult_userid3)
            cursor.execute(newcontact_statement1, values_contact1)
            mydb.commit()
